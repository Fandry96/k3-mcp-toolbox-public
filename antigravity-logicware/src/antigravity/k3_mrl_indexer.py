        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_norm_cache is None:
                ...
                if self.index:
                    ...
                    # Pre-compute 1D squared norms for short matrix
                    m_short = self._matrix_cache[:, :SHORTLIST_DIM]
                    # ⚡ BOLT OPTIMIZATION:
                    # Store 1D squared norms instead of a fully normalized NxD matrix.
                    # This saves memory and aligns with raw dot product scaling at query time.
            ...
            m_short_sq_norm = self._matrix_short_norm_cache
            ...
            # ⚡ BOLT OPTIMIZATION: Compute raw dot product first and scale by 1D norms.
            q_short_sq_norm = np.dot(q_short, q_short)
            raw_scores_short = np.dot(matrix[:, :SHORTLIST_DIM], q_short)
            scores_short = raw_scores_short / (np.sqrt(m_short_sq_norm * q_short_sq_norm) + 1e-9)
            ...
            # ⚡ BOLT OPTIMIZATION:
            # Avoid allocating an intermediate normalized N x D matrix.
            # Compute raw dot product first and scale by 1D norms.
            ...
            scores_full = raw_scores_full / (np.sqrt(m_sq_norms * q_sq_norm) + 1e-9)
