        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_sq_norms_cache is None:
                ...
                if self.index:
                    ...
                    # Pre-compute short matrix 1D squared norms
            ...
            # Unconditionally define m_short for use outside the cache-miss block
            m_short = matrix[:, :SHORTLIST_DIM] if len(matrix) > 0 else np.array([])
            ...
            scores_full = raw_scores_full / ((np.sqrt(m_full_sq_norms) + 1e-9) * (np.sqrt(q_sq_norm) + 1e-9))
