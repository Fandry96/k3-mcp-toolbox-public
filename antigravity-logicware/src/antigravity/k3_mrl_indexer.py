        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_sq_norms_cache is None:
                ...
                if self.index:
                    ...
                    # Pre-compute squared norms for short matrix (1D array instead of NxD)
            ...
            # ⚡ BOLT OPTIMIZATION: Compute cosine similarity using raw dot products
            # scaled by 1D squared norms to avoid allocating NxD normalized matrices.
            q_sq_norm = np.dot(q_short, q_short)
            raw_dot_short = np.dot(m_short, q_short)
            scores_short = raw_dot_short / ((np.sqrt(m_short_sq_norms) + 1e-9) * (np.sqrt(q_sq_norm) + 1e-9))
            ...
            # ⚡ BOLT OPTIMIZATION: Use 1D squared norms for similarity scaling
