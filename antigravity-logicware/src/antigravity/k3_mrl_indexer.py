        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_norm_cache is None:
                ...
                if self.index:
                    ...
                    # ⚡ BOLT OPTIMIZATION:
                    # Pre-compute 1D norms for short matrix instead of fully normalized N x D matrix
                    # np.einsum is used to calculate squared norms extremely fast
                    m_short = self._matrix_cache[:, :SHORTLIST_DIM]
                    self._matrix_short_norm_cache = np.sqrt(np.einsum('ij,ij->i', m_short, m_short))
                else:
            ...
            m_short_norms = self._matrix_short_norm_cache
            ...
            # ⚡ BOLT OPTIMIZATION:
            # Fast cosine similarity using raw dot product scaled by 1D norms
            raw_scores_short = np.dot(matrix[:, :SHORTLIST_DIM], q_short)
            q_short_norm = np.sqrt(np.dot(q_short, q_short))
            scores_short = raw_scores_short / ((m_short_norms * q_short_norm) + 1e-9)
            ...
            # ⚡ BOLT OPTIMIZATION:
            # Fast cosine similarity using raw dot product scaled by 1D norms
            # Avoids O(N*D) memory allocations for normalized matrices
            raw_scores_full = np.dot(m_full_subset, q_vec)
            m_full_norms = np.sqrt(np.einsum('ij,ij->i', m_full_subset, m_full_subset))
            q_full_norm = np.sqrt(np.dot(q_vec, q_vec))
            scores_full = raw_scores_full / ((m_full_norms * q_full_norm) + 1e-9)
