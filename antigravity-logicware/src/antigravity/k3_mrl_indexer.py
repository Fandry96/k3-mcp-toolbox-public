        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_sq_norms_cache is None:
                ...
                if self.index:
                    ...
                    # ⚡ BOLT OPTIMIZATION:
                    # Cache 1D squared norms instead of a fully normalized matrix to save memory.
            ...
            # ⚡ BOLT OPTIMIZATION:
            # Use cached 1D squared norms to scale the raw dot product instead of NxD matrix normalization.
            q_short_sq_norm = np.dot(q_short, q_short)
            m_short = matrix[:, :SHORTLIST_DIM]
            raw_dot_short = np.dot(m_short, q_short)
            scores_short = raw_dot_short / (np.sqrt(m_short_sq_norms * q_short_sq_norm) + 1e-9)
            ...
            # ⚡ BOLT OPTIMIZATION:
            # Replaced NxD matrix normalization with raw dot products scaled by 1D squared norms.
            # Avoids large memory allocations and speeds up computation significantly.
            m_sq_norms = np.einsum('ij,ij->i', m_full_subset, m_full_subset)
            q_sq_norm = np.dot(q_vec, q_vec)
            raw_dot = np.dot(m_full_subset, q_vec)
            scores_full = raw_dot / (np.sqrt(m_sq_norms * q_sq_norm) + 1e-9)
