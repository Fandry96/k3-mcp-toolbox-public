...
class MatryoshkaIndexer:
    def __init__(
        ...
        self._matrix_short_sq_norms_cache = None
    ...
    def search(self, query: str, top_k: int = 5):
        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_sq_norms_cache is None:
                # Rebuild cache
                self._paths_cache = list(self.index.keys())
                if self.index:
                    self._matrix_cache = np.stack([d["vector"] for d in self.index.values()])

                    # ⚡ BOLT OPTIMIZATION: Cache 1D squared norms instead of NxD normalized matrix.
                    # This saves significant memory and avoids the expensive np.linalg.norm allocation.
                    # We compute similarity later using raw dot product scaled by these 1D norms.
                    m_short = self._matrix_cache[:, :SHORTLIST_DIM]
                    self._matrix_short_sq_norms_cache = np.einsum('ij,ij->i', m_short, m_short)
                else:
                    self._matrix_cache = np.array([])
                    self._matrix_short_sq_norms_cache = np.array([])

            paths = self._paths_cache
            matrix = self._matrix_cache
            m_short_sq_norms = self._matrix_short_sq_norms_cache
            ...
            # ⚡ BOLT OPTIMIZATION: Compute cosine similarity using raw dot product scaled by 1D norms.
            # Avoids O(N*D) memory allocations from matrix normalization in hot path.
            q_short_sq_norm = np.sum(q_short * q_short)
            raw_scores_short = np.dot(matrix[:, :SHORTLIST_DIM], q_short)
            scores_short = raw_scores_short / (np.sqrt(m_short_sq_norms * q_short_sq_norm) + 1e-9)
            ...
            # ⚡ BOLT OPTIMIZATION: Avoid NxD normalized matrix allocation for reranking.
            # Use raw dot product scaled by 1D squared norms.
            m_full_sq_norms = np.einsum('ij,ij->i', m_full_subset, m_full_subset)
            q_full_sq_norm = np.sum(q_vec * q_vec)
            raw_scores_full = np.dot(m_full_subset, q_vec)
            scores_full = raw_scores_full / (np.sqrt(m_full_sq_norms * q_full_sq_norm) + 1e-9)
