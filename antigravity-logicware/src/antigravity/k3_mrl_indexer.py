        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_sq_norms_cache is None:
                ...
                if self.index:
                    ...
                    # ⚡ BOLT OPTIMIZATION:
                    # Cache 1D squared norms instead of the full NxD normalized matrix.
                    # This saves significant memory and aligns with the raw dot product optimization.
            ...
            # Exact functional match: scale dot product by (||m|| + 1e-9) * (||q|| + 1e-9)
            denom_short = (np.sqrt(m_short_sq_norms) + 1e-9) * (np.sqrt(q_short_sq_norm) + 1e-9)
            scores_short = raw_scores_short / denom_short
            ...
            scores_full = raw_scores_full / denom_full
