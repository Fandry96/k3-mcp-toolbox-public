        ...
        try:
            ...
            if self._matrix_cache is None or self._paths_cache is None or self._matrix_short_sq_norms_cache is None:
                ...
                if self.index:
                    ...
                    # ⚡ BOLT OPTIMIZATION: Use np.einsum for squared norms instead of intermediate NxD matrices
            ...
            # Ensure m_short is defined outside cache check for downstream calculations
            ...
            scores_short = raw_scores_short / (
                (np.sqrt(m_short_sq_norms) + 1e-9) * (np.sqrt(q_short_sq_norm) + 1e-9)
            )
            ...
            scores_full = raw_scores_full / (
                (np.sqrt(m_full_sq_norms) + 1e-9) * (np.sqrt(q_full_sq_norm) + 1e-9)
            )
