...
class MatryoshkaIndexer:
    ...
    def search(self, query: str, top_k: int = 5):
        ...
        try:
            ...
            # Normalize with 1D squared norms to avoid NxD memory allocation
            q_short_sq_norm = np.dot(q_short, q_short)
            denom_short = (np.sqrt(m_short_sq_norms) + 1e-9) * (np.sqrt(q_short_sq_norm) + 1e-9)
            scores_short = np.dot(m_short, q_short) / denom_short
            ...
            q_vec_sq_norm = np.dot(q_vec, q_vec)

            denom_full = (np.sqrt(m_full_sq_norms) + 1e-9) * (np.sqrt(q_vec_sq_norm) + 1e-9)
            scores_full = np.dot(m_full_subset, q_vec) / denom_full
