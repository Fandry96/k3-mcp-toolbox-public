...
class MatryoshkaIndexer:
    ...
    def search(self, query: str, top_k: int = 5):
        ...
        try:
            ...
            if getattr(self, '_matrix_short_sq_norms_cache', None) is None or self._matrix_cache is None or self._paths_cache is None:
                # Rebuild cache
                self._paths_cache = list(self.index.keys())
                if self.index:
                    self._matrix_cache = np.stack([d["vector"] for d in self.index.values()])

                    # ⚡ BOLT OPTIMIZATION: Compute 1D squared norms instead of full NxD matrix
            ...
            # ⚡ BOLT OPTIMIZATION: Use raw dot product scaled by cached 1D squared norms
            q_short_sq_norm = np.dot(q_short, q_short)
            scores_short = np.dot(matrix[:, :SHORTLIST_DIM], q_short) / (
                np.sqrt(m_short_sq_norms * q_short_sq_norm) + 1e-9
            )
            ...
            # ⚡ BOLT OPTIMIZATION: Dynamic subset 1D squared norm scaling
            m_full_sq_norms = np.einsum('ij,ij->i', m_full_subset, m_full_subset)
            q_full_sq_norm = np.dot(q_vec, q_vec)
            scores_full = np.dot(m_full_subset, q_vec) / (
                np.sqrt(m_full_sq_norms * q_full_sq_norm) + 1e-9
            )
