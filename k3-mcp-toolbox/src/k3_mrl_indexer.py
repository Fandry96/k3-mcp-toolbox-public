...
class MatryoshkaIndexer:
    ...
    def search(self, query: str, top_k: int = 5):
        ...
        try:
            ...
            # ⚡ BOLT OPTIMIZATION: Avoid NxD memory allocations and O(N) log N scaling.
            # Compute 1D squared norms and scale the raw dot product.
            m_short_sq_norms = np.einsum('ij,ij->i', m_short, m_short)
            q_short_sq_norm = np.dot(q_short, q_short)
            scores_short = np.dot(m_short, q_short) / (
