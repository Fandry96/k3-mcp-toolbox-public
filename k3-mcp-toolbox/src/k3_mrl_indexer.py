...
class MatryoshkaIndexer:
    ...
    def walk_files(self) -> List[Path]:
        ...
        # ⚡ BOLT OPTIMIZATION: convert to tuple for fast native string endswith check
        ext_tuple = tuple(extensions)

        for root, dirs, files in os.walk(self.target_dir):
            dirs[:] = [d for d in dirs if d not in skip_dirs]
            for file in files:
                # ⚡ BOLT OPTIMIZATION: string method avoids object instantiation overhead of Path(file)
